...
@Component({
  template: `
    ...
    <div>
        <CourseSessionList [params]="{semesterLevelId: semesterLevel.id}" 
...
export class SemesterLevelCourseSessionsPage {
  ...
  addAll() {
    this.service.addCourseSession({semesterLevel: this.semesterLevel}).subscribe(item => {
      if(item) {
